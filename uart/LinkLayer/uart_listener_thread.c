...
void* config_channel_Listener(void *pvoid)
{
  ...
  #if Debug_ubuntu_Level	== 1
  pUartPort->FreqValue				=	CHANNEL_CONF_VALUE;
  pUartPort->AirRate					=	config_chn_AirRate;
  pUartPort->GateWayAddress		=	config_chn_GateWayHalAddress;
  #endif
  
	pUartPort->channelNumber		=	ConfigLoraChn;
...
//			perror(" wait semaphore failure");
		      if(uart_read_wait_time(pUartPort->clt_uart, waitTimes) == 0)
					{
						#if (Debug_Level == 1)
            //printf("%s,%d,config %s read timeout\n",__FILE__,__LINE__,uart);
		        #endif
            ...
            //-----------	analysis uart frame	---------------
            //wait for mutex_lock rewrite
            pVirtualSocket->pContext.p 	 	=  (void*) databuf;							// because register log pack use server response context.
            pVirtualSocket->pContext.len  =    idataLen;		
						res	=	config_command_analysis(pUartPort,pVirtualSocket);		
				}
			}//end while(2)
