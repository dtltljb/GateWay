...
void* fourth_channel_Listener(void *pvoid)
{
...
#else
  char *uart = "/dev/ttymxc4";
#endif

  UartPort *pUartPort		=	&mUartPort[FourthUartNum];
	pUartPort->h_io				=	uart;
	pUartPort->bps		=	9600;
	pUartPort->bit		=	8;
	pUartPort->check	=	'N';
  ...
  #if Debug_ubuntu_Level	== 1
  pUartPort->FreqValue				=	CHANNEL_FOUR_VALUE;
  pUartPort->AirRate					=	config_chn_AirRate;
  pUartPort->GateWayAddress		=	fourth_chn_GateWayHalAddress;
  #endif

	pUartPort->channelNumber		=	FourthLoraChn;	
	pUartPort->deviceStates			=	WorkStatesIDLE;
	if ( pvoid == NULL )
    printf("%s,line:%d fourth_channel_Listener run... \n",__FILE__,__LINE__);
