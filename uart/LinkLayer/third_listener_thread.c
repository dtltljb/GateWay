...
/*
*		third_channel_Listener
******************************************************************/
void* third_channel_Listener(void	*pvoid)
{
//--------------	Parament initialize	----------------
#if Debug_ubuntu_Level	== 1
    char *uart = "/dev/ttyUSB3";
#else
    char *uart = "/dev/ttymxc3";
#endif

  UartPort *pUartPort		=	&mUartPort[ThirdUartNum];
	pUartPort->h_io				=	uart;
	pUartPort->bps		=	9600;
	pUartPort->bit		=	8;
	pUartPort->check	=	'N';
  ...
  #if Debug_ubuntu_Level	== 1
  pUartPort->FreqValue				=	CHANNEL_THREE_VALUE;
  pUartPort->AirRate					=	third_chn_AirRate;
  pUartPort->GateWayAddress		=	third_chn_GateWayHalAddress;
  #endif

	pUartPort->channelNumber		=	ThirdLoraChn;	
	pUartPort->deviceStates			=	WorkStatesIDLE;
	    if ( pvoid == NULL )
       printf("%s,line:%d third_channel_Listener run... \n",__FILE__,__LINE__);
		//-----------		public part	----------------------------
  uint8_t	waitTimes	=	5;
