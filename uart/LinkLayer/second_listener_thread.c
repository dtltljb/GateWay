...
/*
*		second_channel_Listener
******************************************************************/
#define				debug_second_channel			(1)
...
//--------------	Parament initialize	----------------
  ...
  const char *uart = "/dev/ttyUSB2";
#else
  const char *uart = "/dev/ttymxc2";
#endif
  UartPort *pUartPort		=	&mUartPort[SecondUartNum];
	pUartPort->h_io				=	uart;
	pUartPort->bps		=	9600;
	pUartPort->bit		=	8;
	pUartPort->check	=	'N';
  ...
  #if Debug_ubuntu_Level	== 1
  pUartPort->FreqValue				=	CHANNEL_TWO_VALUE;
  pUartPort->AirRate					=	second_chn_AirRate;
  pUartPort->GateWayAddress		=	second_chn_GateWayHalAddress;
  #endif
  
	pUartPort->channelNumber		=	SecondLoraChn;	
	pUartPort->deviceStates			=	WorkStatesIDLE;
	    if ( pvoid == NULL )
         printf("%s,line:%d second_channel_Listener run... \n",__FILE__,__LINE__);
	//-----------		public part	----------------------------
  uint8_t	waitTimes	=	5;
